...
class pac_man():
	...
	#renvoi deux tuples pour les coord de la 1er et derniere case plus le score du chemin 	
		...
		while 1:
			...
			if ma_case.indesirable != 0:
				score -= 100
				return [(self.x, self.y), (ma_case.x, ma_case.y), score]
...
while True:
	...
	for row in my_map:
		for my_case in row:
			if my_case.indesirable != 0:
				if my_case.indesirable == 4:
					my_case.indesirable = 0
				else:
					my_case.indesirable += 1 
	...
	for i in range(visible_pac_count):
		...
		if TURN == 0:
			...
			#ET DANS TOUT LES CAS J'AJOUTE A MA MAP
	...
	for pac in myPacList:
		...
		#SINON SI HAVE TARGET
			...
			print("HUNT:", pac.target, file=sys.stderr)
