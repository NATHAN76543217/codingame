...
def get_snuffle_in_danger(snaffle_list):
	cnt = 0
	for snaffle in snaffle_list:
...
def throw_ball(wizard, gx, gy):
	print("Debug THROW", file=sys.stderr)
	print("THROW", gx, gy, "400")
...
#Pour chaque magicien
	...
	for wizard in wizard_list:
		...
		if wizard["id"] % 2 != 0 and get_snuffle_in_danger(snaffle_list) > 0:
			wizard["mode"] = "DEF"
		else:
			wizard["mode"] = "AT"
		...
		print("wiz stat =", wizard["state"], file=sys.stderr)
		if wizard["mode"] == "AT":
			if my_magic > 75:
				...
				print("Debug throw ball\n", file=sys.stderr)
				throw_ball(wizard, goal_ennemy_x, goal_ennemy_y)
			else:
				find = 0
				...
				if ennemy_zone.is_in(wizard["x"], wizard["y"]):
					...
					if snaffle is not None and close_ennemy_zone.is_in(snaffle["x"], snaffle["y"]) and find == 0:
						...
						print("MOVE", snaffle["x"], snaffle["y"], "120")
						find = 1
					else:
				...
				if find == 0:
					...
					if closest is not None:
						print("Debug wizard:", wizard["id"], "chasse sf:", closest["id"], file=sys.stderr)
			...
			if wizard["state"] == 1:
				print("Debug throw ball def\n", file=sys.stderr)
				throw_ball(wizard, goal_ennemy_x, goal_ennemy_y)
			else:
				if closest is not None and my_zone.is_in(wizard["x"], wizard["y"]) == 1:
					target_id[wizard["id"] % 2] = snaffle["id"]
					print("MOVE", closest["x"], closest["y"], "100")
					...
					if closest is not None:
						...
						print("MOVE", wizard["x"] + 1, wizard["y"] + 1, "100")